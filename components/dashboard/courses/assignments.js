import { CheckCircle2, Circle, ChevronRight } from "lucide-react";
export default async function CourseAssignments({ assignments, userId }) {
  return (
    <section className="mb-8">
      <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">
        Assignments
      </h2>

      {/* Pending */}
      <PendingCourseAssignments assignments={assignments} userId={userId} />

      {/* Completed */}

      <CompletedCourseAssignments assignments={assignments} userId={userId} />
    </section>
  );
}

export function PendingCourseAssignments({ assignments, userId }) {
  const pendingAssignment = assignments.filter((assignment) => {
    if (new Date() < new Date(assignment?.dueDate)) {
      return !assignment.studentsSubmitted.includes(userId);
    }
  });
  console.log(pendingAssignment);

  return (
    <>
      {assignments.length > 0 && (
        <div className="mb-4">
          <h3 className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">
            Pending ({pendingAssignment.length})
          </h3>
          <div className="space-y-2">
            {pendingAssignment.map(({ title, dueDate }, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"
              >
                <Circle className="h-5 w-5 text-slate-300 dark:text-slate-600 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-900 dark:text-white truncate capitalize">
                    {title}
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Due:{" "}
                    {new Date(dueDate).toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                      year: "numeric",
                    })}
                  </p>
                </div>
                <ChevronRight className="h-4 w-4 text-slate-400" />
              </div>
            ))}
          </div>
        </div>
      )}
    </>
  );
}

export function CompletedCourseAssignments({ assignments, userId }) {
  const completedAssignment = assignments.filter((assignment) => {
    return assignment?.studentsSubmitted.includes(userId);
  });

  return (
    <>
      {completedAssignment.length > 0 && (
        <div>
          <h3 className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">
            Completed ({completedAssignment.length})
          </h3>
          <div className="space-y-2">
            {completedAssignment.map((a, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"
              >
                <CheckCircle2
                  className={`h-5 w-5 shrink-0 text-sky-600 dark:text-sky-400`}
                />
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-900 dark:text-white truncate">
                    {a.title}
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Submitted: {a.dueDate}
                  </p>
                </div>
              </div>
            ))}
          </div>
        </div>
      )}
    </>
  );
}
